...
- 出勤・営業の時刻 → 繁忙期などの単発の時刻（例「朝2時に出勤」）は書いてよいが、**通常時との差（「いつもより4時間早い」等）は書かない**。差を出すと平常時の出勤時刻＝実質的な営業サイクルが逆算できてしまうため。「いつもより何時間も早く」のように幅を持たせる。
...
## 表示の堅牢性（絶対に守る）
- **コンテンツの表示をJSに依存させない**。フェードイン等で要素を隠す場合は、`<head>` で `html` に付与するクラス（`.js`）でゲートし、**JSが動かない環境では最初から表示される**ようにする。JSが1つ失敗しただけでページが白紙になる構造を作らない。
  - 2026-08-16、トップページが `.reveal{opacity:0}` で本文11ブロックを隠しJSでしか表示していなかったため、スマホで**ヘッダー以外が真っ白**になる事故が発生した（修正済み）。
  - 隠す処理を書くときは必ず「非対応・例外発生・一定時間経過」のフォールバックを用意し、最後は無条件で表示させる。
  - `prefers-reduced-motion: reduce` ではアニメーションせず即時表示する。
- **描画に関わる不具合の切り分け順序**：①一部でも描画されているか（ヘッダーが出ていれば配信・CSS・フォントは正常）→ ②`curl` でHTTPステータス・サイズ・TTFB → ③実ブラウザで console / network / computed style を確認。見た目の重さから原因を推測しない。
- Webフォントの `<link>` は `media="print" onload="this.media='all'"` ＋ `<noscript>` フォールバックで**描画をブロックしない形**にする（全ページ適用済み）。日本語フォントはウェイトを増やすほど定義CSSが肥大するため、ウェイト追加は慎重に。
...
これを怠ると直リンクではアクセスできてもサイト内の一覧に載らない。
   **【関連リンクの鉄則】** 関連記事リンク（`class="related__item"` など）には**公開済みの記事にのみリンクする**。執筆予定・下書き中のslugをリンクに書かない。記事のslugが変わった場合はリンク元も同時に更新する。未公開リンクの放置はGSC 404エラーになる。
...
## 公開済み記事一覧（最終更新：2026-08-16・45記事）
| カテゴリ | slug | タイトル |
|---|---|---|
| kaigyo | shokuhin-hyoji | 弁当の食品表示ラベルに何を書くか——原材料は重量の多い順、アレルゲンは50種を一度網羅、指摘を受けたのは文字サイズだった |
| keiei | hanbouki | 大型連休の売上は約2倍。増えるのは弁当ではなく別注弁当——弁当屋が5日前に受注を締め、朝2時に出勤する繁忙期の回し方 |
| keiei | menu-kousei | 弁当屋のメニューは固定3品＋日替わり5品——品数は「毎日ローテーションが組める数」に落ち着いた |
| keiei | natsu-eisei | 夏の弁当づくりは「おいしい」より「安全」——食中毒ゼロ3年の弁当屋がやっている加熱・小ロット製造・保冷の声かけ |
| shikumi | gas-line-tsuchi | 毎日10時半、翌日の予約一覧がLINEに届く——GASで通知を自動化し、Gmail照合で登録漏れまで見張る弁当屋の仕組み【コード公開】 |
| keiei | shiire-tsukaiwake | 弁当屋の仕入れ先は5系統——肉と調味料は卸・米は農家直・塩砂糖はコストコという使い分けと、肉の卸を変えた話 |
| kaigyo | eigyo-kyoka | 飲食店営業許可とそうざい製造業許可の両方を取った——弁当屋が2つの許可を選んだ理由と施設基準・検査当日の全記録 |
| keiei | shiire-neage | 仕入れ値が2倍になった3年間——弁当屋が食材高騰に取ってきた選択と、まだ続く値上がり（※進行中・追記予定） |
...
| review | tiger-suihanki | タイマーのない業務用炊飯器をAlexaで予約炊飯した話——タイガー2升炊きを1年以上使った正直なレビュー |
...
| money | konetsuhi | 光熱費2万円台から7万円近くへ——弁当屋3年、生産数と光熱費は比例して増えるという現実 |
...
| kaigyo | shokuhin-eisei | 食品衛生責任者の講習を受けた——申し込んだら4か月待ちで、試験なしで修了証書をもらう話 |
